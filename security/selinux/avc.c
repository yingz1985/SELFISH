...
int sendmsg(socket *sock,size_t size,char* sendbuf)    //sendbuf should already been initialized
{
    ...
    ret=kernel_sendmsg(sock,&msg,&vec,1,vec.iov_len); //send message
    if(ret<0)
    {
...
u8 retrieve(u32 ssid,u32 tsid,u16 objc)    //returns 8 bits, only 6 are needed
{
    ...
    /*    *sendbuf = ssid;
     *(sendbuf+4) = tsid;
     *(sendbuf+8) = 0xffff;    //0xff for retrieval
     *(sendbuf+10)= objc;
     */
...
static inline struct avc_node *avc_search_node(struct selinux_state *state,struct selinux_avc *avc,
                                               u32 ssid, u32 tsid, u16 tclass)
{
    struct avc_node *node = NULL;
    
    u8 allowed = retrieve(ssid, tsid, tclass);
    if (allowed & 0x20)    //if hit
    {
        //hit and retrieve the permission bits + store in allowed field
        //with the bits returned we must Initialize the avc_decision fields
        node = avc_alloc_node(avc);
        if (node)
        {
            //fill in entry partially..don't know if it works yet yikes
            //struct hlist_head *head;
            //spinlock_t *lock;
            struct av_decision avd;
            
            //can also just call avc_init from services.c
            avd.allowed = allowed & 0x1f;       //5 bit permissions
            avd.auditallow = 0;
            avd.auditdeny = 0xffffffff;
            avd.seqno = 0x0;//state->ss->latest_granting; //not sure if this is useful
            //if we determine that seqno
            avd.flags = 0; //0x1 is permission, 0x0 is enforcing
            
            
            //only the allowed field is populated, assuming that enforcing is 0x0
            avc_node_populate(node, ssid, tsid, tclass, &avd);
            //avd values get overwritten after the function terminates
            //should be fine
            
            //head = &avc->avc_cache.slots[hvalue];
            //lock = &avc->avc_cache.slots_lock[hvalue];
            //do we need to use locks here? access to hardware
            
            //other fields are minor
            
        }
    }
    
    return node;    //node==NULL by default if result not good
